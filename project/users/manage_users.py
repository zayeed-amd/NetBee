...
def users():
    ...
    if request.method == "POST":
        ...
        new_user = User(username=username, email=email, password=generate_password_hash(password, method='sha256'), permission_id=permission_id)
    ...
    return render_template(template_name_or_list='users/users.html', users_info=users_info, current_user=flask_login.current_user, permissions=permissions)
...
def update(id):
    ...
    if request.method == "POST":
        ...
        return render_template(template_name_or_list='users/update.html', user=user, permissions=permissions)
        # return redirect(url_for('users_bp.users'), user=user)
    try:
        db.session.commit()
        return redirect(url_for('users_bp.users'))
    finally:
